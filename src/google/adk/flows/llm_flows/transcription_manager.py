...
class TranscriptionManager:
  ...
  async def _create_and_save_transcription_event(
    ...
    try:
      ...
      logger.debug(
          ...
          author,
          transcription.text if hasattr(transcription, 'text') else 'audio transcription'
      )
  ...
  def get_transcription_stats(self, invocation_context: InvocationContext) -> dict[str, int]:
