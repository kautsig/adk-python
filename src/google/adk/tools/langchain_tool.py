...
class LangchainTool(FunctionTool):
  ...
  def __init__(
    ...
    # Check if the tool has a 'run' method, '_run' method, is a proper langchain tool, or is a callable function
    if (not hasattr(tool, 'run') and 
        not hasattr(tool, '_run') and 
        not isinstance(tool, StructuredTool) and
        not callable(tool)):
      raise ValueError(
          "Tool must be a Langchain tool, have a 'run' or '_run' method, or be a callable function."
      )
    ...
    if isinstance(tool, StructuredTool):
      ...
      # Handle raw callable functions (both sync and async)
      func = tool
    super().__init__(func)
