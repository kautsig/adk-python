...
class AdkWebServer:
  ...
  def __post_init__(self):
    """For propeties we want to allow being modified from callbacks."""
  ...
  def get_fast_api_app(
    ...
    async def run_eval(
      ...
      if req.eval_ids:
        eval_cases = [
            e for e in eval_set.eval_cases if e.eval_id in req.eval_ids
        ]
        eval_set_to_evals = {eval_set_id: eval_cases}
      else:
        logger.info(
            "Eval ids to run list is empty. We will run all eval cases."
        )
      ...
      try:
        async for eval_case_result in run_evals(
          ...
          eval_case_result.session_details = (
              await self.session_service.get_session(
                  app_name=app_name,
                  user_id=eval_case_result.user_id,
                  session_id=eval_case_result.session_id,
      ...
      self.eval_set_results_manager.save_eval_set_result(
    ...
    #     self.agent_loader.load_agent(agent_name=req.agent_name)
    #     return True
