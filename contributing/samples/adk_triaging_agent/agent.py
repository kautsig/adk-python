...
def assign_issue_to_owner(issue_number: int, label: str) -> dict[str, Any]:
  """Assign the given issue number to the specified owner.

  Args:
    issue_number: issue number of the Github issue.
    label: label added to the issue.

  Returns:
    The the status of this request, with the assigned owner when successful.
  """
...
root_agent = Agent(
    ...
    instruction=f"""
      ...
      After you label the issue, you should assign the issue to the owner of the label using the `assign_issue_to_owner` tool.
    ...
    """,
    tools=[list_unlabeled_issues, add_label_to_issue, assign_issue_to_owner],
)
