...
def test_supported_models():
  ...
  assert models[3] == "computer-use-exp"
  assert (
      models[4]
...
@pytest.mark.parametrize(
    "model_name, expected_system_instruction",
    [
        ("gemini-1.5-flash", "You are a helpful assistant"),
        ("computer-use-exp", None),
        (
            "projects/my-project/locations/us-central1/publishers/google/models/computer-use-exp",
            None,
        ),
        (
            "projects/my-project/locations/us-central1/publishers/google/models/gemini-1.5-flash",
            "You are a helpful assistant",
        ),
    ],
)
def test_preprocess_request_computer_use_model_removes_system_instruction(
    gemini_llm: Gemini,
    model_name: str,
    expected_system_instruction: str,
):
  """
  Tests that _preprocess_request correctly removes system instruction for computer-use models.

  - For computer-use-exp models (both short and long form), system instruction should be removed
  - For other models, system instruction should be preserved
  """
  # Arrange: Create a request with system instruction
  ...
  # Act: Run the preprocessing method
  gemini_llm._preprocess_request(llm_request)

  # Assert: Check if system instruction was correctly processed
  assert llm_request.config.system_instruction == expected_system_instruction
