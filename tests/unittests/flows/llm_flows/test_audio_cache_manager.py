...
class TestAudioCacheManager:
    ...
    async def test_cache_input_audio(self):
        ...
        # Initially no cache
        assert invocation_context.input_audio_cache is None
        
        # Cache audio
        self.manager.cache_input_audio(invocation_context, audio_blob)
        
        # Verify cache is created and populated
        assert invocation_context.input_audio_cache is not None
        assert len(invocation_context.input_audio_cache) == 1
        
        entry = invocation_context.input_audio_cache[0]
    ...
    async def test_cache_output_audio(self):
        ...
        # Initially no cache
        assert invocation_context.output_audio_cache is None
        
        # Cache audio
        self.manager.cache_output_audio(invocation_context, audio_blob)
        
        # Verify cache is created and populated
        assert invocation_context.output_audio_cache is not None
        assert len(invocation_context.output_audio_cache) == 1
        
        entry = invocation_context.output_audio_cache[0]
    ...
    async def test_multiple_audio_caching(self):
        ...
        for i in range(2):
            audio_blob = types.Blob(data=f'output_{i}'.encode(), mime_type='audio/wav')
            self.manager.cache_output_audio(invocation_context, audio_blob)
        
        # Verify all chunks are cached
        assert len(invocation_context.input_audio_cache) == 3
        assert len(invocation_context.output_audio_cache) == 2
    ...
    async def test_flush_caches_both(self):
        ...
        self.manager.cache_input_audio(invocation_context, input_blob)
        self.manager.cache_output_audio(invocation_context, output_blob)
        
        # Flush caches
        await self.manager.flush_caches(invocation_context)
        
        # Verify caches are cleared
        assert invocation_context.input_audio_cache == []
        assert invocation_context.output_audio_cache == []
    ...
    def test_get_cache_stats_empty(self):
        ...
        invocation_context.input_audio_cache = None
        invocation_context.output_audio_cache = None
    ...
    async def test_get_cache_stats_with_data(self):
        ...
        self.manager.cache_input_audio(invocation_context, input_blob1)
        self.manager.cache_input_audio(invocation_context, input_blob2)
        self.manager.cache_output_audio(invocation_context, output_blob)
    ...
    async def test_filename_uses_first_chunk_timestamp(self):
        ...
        first_entry = AudioCacheEntry(
            role='user',
            data=types.Blob(data=b'first_chunk', mime_type='audio/pcm'),
            timestamp=first_timestamp
        )
        
        second_entry = AudioCacheEntry(
            role='user', 
            data=types.Blob(data=b'second_chunk', mime_type='audio/pcm'),
            timestamp=second_timestamp
        )
        
        invocation_context.input_audio_cache.extend([first_entry, second_entry])
        ...
        assert f"input_audio_{expected_timestamp_ms}.pcm" == filename
        ...
        assert filename.startswith(f"input_audio_{expected_timestamp_ms}") 